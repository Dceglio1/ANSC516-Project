...
if(!dir.exists("output/Monthly Flush/taxa"))
  dir.create("output/Monthly Flush/taxa")
...
physeq <- qza_to_phyloseq(
  features="Qiime Outputs/Monthly Flush/rarefied_table.qza",
  tree="Qiime Outputs/QZA/rooted-tree.qza",
  taxonomy = "Qiime Outputs/QZA/taxonomy.qza",
  metadata = "AliyaMetaDataMonthFlush.tsv"
)
...
for (i in 1:nrow(tax.clean)){
  if (tax.clean[i,2] == ""){
    kingdom <- paste("uncl_", tax.clean[i,1], sep = "") #Change uncl_ to Kingdom if want
...
my_level <- c("Phylum", "Family", "Genus") #Will make 3 graphs
my_column <- "StagType"  #this is the metadata column that we will use in the taxa barplot

metadata <- filter(metadata, !my_column %in% c(''))
meta.physeq = sample_data(metadata)
physeq_bar_plot = phyloseq(OTU.physeq, tax.physeq, meta.physeq)
...
for(ml in my_level){
  ...
  ggsave(paste0("output/Monthly Flush/taxa/", ml, "BarPlot_", my_column, ".png"), height = 5, width = 4)
}
...
diagdds = phyloseq_to_deseq2(physeq_deseq, ~ StagType)
...
run_deseq2 <- function(my_factor, x, y){
  ...
  ggsave(paste0("output/Monthly Flush/taxa/DESeq2-", my_contrast[2], "-", my_contrast[3], ".png"), DESeq_fig, height = 5, width = 11)
}

run_deseq2("StagType", "Daily.CP", "Daily.HE")
run_deseq2("StagType", "Weekly.CP", "Weekly.HE")
run_deseq2("StagType", "Biweekly.CP", "Biweekly.HE")
run_deseq2("StagType", "Monthly.CP", "Monthly.HE")
run_deseq2("StagType", "Daily.CP", "Weekly.CP")
run_deseq2("StagType", "Daily.CP", "Biweekly.CP")
run_deseq2("StagType", "Daily.CP", "Monthly.CP")
run_deseq2("StagType", "Weekly.CP", "Biweekly.CP")
run_deseq2("StagType", "Weekly.CP", "Monthly.CP")
run_deseq2("StagType", "Biweekly.CP", "Monthly.CP")
run_deseq2("StagType", "Daily.HE", "Weekly.HE")
run_deseq2("StagType", "Daily.HE", "Biweekly.HE")
run_deseq2("StagType", "Daily.HE", "Monthly.HE")
run_deseq2("StagType", "Weekly.HE", "Biweekly.HE")
run_deseq2("StagType", "Weekly.HE", "Monthly.HE")
run_deseq2("StagType", "Biweekly.HE", "Monthly.HE")
